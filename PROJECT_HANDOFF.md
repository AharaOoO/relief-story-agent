...
- 本地验收证据收集器：`relief-story-agent local-acceptance` 会运行 `compileall`、全量 pytest，并可选收集 `model-check`、run/batch `diagnose`、`smoke-comfyui`，落盘 `command_outputs/`、`local_acceptance_summary.json`、`acceptance_report.json` 和 `ACCEPTANCE_REPORT.md`，方便另一个 AI 或操作者核查当前进度。
