...
- 本地验收证据收集器：`relief-story-agent local-acceptance` 会运行 `compileall`、全量 pytest，并可选收集 `model-check`、run/batch `diagnose`、`smoke-comfyui` 结果，生成可交给另一个 AI 核查的 JSON/Markdown 报告。
- 真实本机 smoke 证据：
