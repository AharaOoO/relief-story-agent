...
const SECRET_FIELDS = [
  { name: 'RUNNINGHUB_CN_API_KEY', label: 'RunningHub 国内站 G2', hint: '.cn 个人/会员 key，仅用于 G2 与任务 API' },
  { name: 'RUNNINGHUB_AI_API_KEY', label: 'RunningHub 国际站 G2', hint: '.ai 个人/会员 key，仅用于 G2 与任务 API' },
  { name: 'RUNNINGHUB_CN_SHARED_API_KEY', label: 'RunningHub 国内站 LLM', hint: '.cn Enterprise-Shared key，仅前六步企业模型使用' },
  { name: 'RUNNINGHUB_AI_SHARED_API_KEY', label: 'RunningHub 国际站 LLM', hint: '.ai Enterprise-Shared key，仅前六步企业模型使用' },
      ...
      >
        ...
        <div className="settings-layout">
          ...
          <div className="settings-content">
            ...
            {tab === 'image' && (
              <div className="settings-section">
                <div><h3>G2 四宫格参考图</h3><p>使用站点个人/会员 key，与前六步企业共享模型 key 分开保存；生成后自动交给 LTX 工作流。</p></div>
                ...
                <div className="field-stack"><span>G2 生图站点</span><div className="segmented-control"><button type="button" className={runDraft.gridImageSite === 'cn' ? 'is-active' : ''} onClick={() => patchRunDraft({ gridImageSite: 'cn' })}>国内站 .cn</button><button type="button" className={runDraft.gridImageSite === 'ai' ? 'is-active' : ''} onClick={() => patchRunDraft({ gridImageSite: 'ai' })}>国际站 .ai</button></div></div>
                ...
                <div className={`inline-notice ${secretStatus[runDraft.gridImageSite === 'cn' ? 'RUNNINGHUB_CN_API_KEY' : 'RUNNINGHUB_AI_API_KEY']?.configured ? '' : 'is-warning'}`}>{secretStatus[runDraft.gridImageSite === 'cn' ? 'RUNNINGHUB_CN_API_KEY' : 'RUNNINGHUB_AI_API_KEY']?.configured ? '当前 G2 站点密钥已配置。' : '当前 G2 站点还没有配置 RunningHub 密钥。'}</div>
              </div>
            )}
