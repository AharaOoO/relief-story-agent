...
Its JSON response keeps the legacy top-level file path keys and also includes `files`, `checks`, `next_commands`, and `next_endpoints` for local launchers and future UI shells. `next_commands.smoke_dry_run` and `next_commands.smoke_real_run` use the generated smoke request.
