...
def build_chief_screenwriter_prompt(
    *,
    ...
    return dedent(
        ...
        创作题目：{idea}
        目标压力场景：{audience_pressure or "由你根据题目合理选择"}
        偏好风格：{preferred_style or "现实、奇幻、Q版、都市、生活流、温柔喜剧均可"}
        偏好系列：{preferred_series or "可从系列库中选择，也可生成独立单集"}
...
def build_deepseek_polish_prompt(draft_payload: dict) -> str:
    return dedent(
        f"""
